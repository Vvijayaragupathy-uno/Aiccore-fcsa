...
`
    ...
    // Parse the JSON response with robust extraction logic
    ...
    // Check if response starts with { - if not, skip direct parsing
    const startsWithJson = text.trim().startsWith('{')
    
    if (startsWithJson) {
      try {
        structuredAnalysis = JSON.parse(text)
        console.log('Successfully parsed balance sheet analysis directly')
      } catch (parseError) {
        console.error('Direct JSON parsing failed despite starting with {:', parseError)
      }
    }
    
    // If direct parsing failed or response doesn't start with JSON, extract it
    if (!structuredAnalysis) {
      ...
      // Method 1: Extract JSON object from anywhere in text (most reliable)
      jsonContent = extractJSON(text)
      if (jsonContent) {
        console.log('Extracted JSON object from text')
      }
      
      // Method 2: Extract from markdown code blocks (fallback)
      if (!jsonContent) {
        let jsonMatch = text.match(/```json\s*([\s\S]*?)\s*```/)
        if (jsonMatch) {
          jsonContent = jsonMatch[1].trim()
          console.log('Found JSON in markdown json block')
        }
      }
      
      // Method 3: Extract from any code block (last resort)
      if (!jsonContent) {
      ...
      if (jsonContent) {
        try {
          ...
          console.error('Extracted content preview:', jsonContent.substring(0, 500))
        }
      }
    }
      
      // If still no success, create fallback structure
      if (!structuredAnalysis) {
