...
/**
 * Check if a value is valid (not NaN, null, undefined - but zero IS valid)
 */
...
/**
 * Check if an array has any meaningful data points (not all undefined/null/NaN)
 */
...
/**
 * Check if data has actual meaningful values (not all zeros)
 */
export function hasNonZeroData(values: (number | null | undefined)[]): boolean {
  return values.some(value => isValidValue(value) && value !== 0)
}

/**
 * Filter out invalid values from an array, but keep zeros
 */
